...
test('4.upwork_job_1', async ({browser})=>
{
    ...
    for (let j = 0; j < count; j++) {
        ...
        // console.log(reviews.split(" ").slice(-1)[0].split("(").slice(-1)[0].split(")")[0]);
        ...
        // data_dict['Company Name'] = textContents[1].trim().split(".")[1];
        ...
        // await newPage.waitForSelector('(//div[@class="appx-average-rating-detail"]//div)[1]');
        ...
        // const rating = await newPage.locator('(//div[@class="appx-average-rating-detail"]//div)[1]').textContent();


        data_dict['Rating'] = rating;


        // fs.writeFileSync(path.resolve(__dirname, 'appexchange.json'), JSON.stringify(data_dict));
        fs.appendFileSync(path.resolve(__dirname, 'appexchange.json'), JSON.stringify(data_dict));
        // await writeJsonFile.sync('appexchange.json', data_dict);
    ...
    // fs.appendFileSync(path.resolve(__dirname, 'appexchange_2.json'), JSON.stringify(data));
